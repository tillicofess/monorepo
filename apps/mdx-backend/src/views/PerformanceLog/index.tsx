...
import type {
	PageLoadTiming,
	PerformanceData,
} from "@/views/PerformanceLog/types/performanceType";
import {
	METRIC_KEYS,
	performanceMetrics,
} from "@/views/PerformanceLog/types/performanceType";
...
const PerformanceLog: React.FC = () => {
	...
	const { data: urls } = useSWR(
		selectedApp ? `/errorLogs/urls/${selectedApp}` : null,
		fetcher,
	);

	const { data: performanceLogs, isLoading: performanceLoading } = useSWR<
		PerformanceData[]
	>(
