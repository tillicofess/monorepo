...
function extractFirstImageUrl(content) {
	const markdownImageRegex =
		/!\[.*?\]\((https?:\/\/[^\s)]+|\/largeFile\/download\/[a-f0-9-]+)\)/i;
...
export const publishArticle = async (req, res) => {
	...
	if (!title || !content) {
		return res
			.status(400)
			.json({ code: 1, message: "Title and content are required." });
	}
	if (title.length > 255) {
		return res
			.status(400)
			.json({ code: 1, message: "Title cannot exceed 255 characters." });
	}
...
export const getArticleById = async (req, res) => {
	...
	if (!id) {
		return res
			.status(400)
			.json({ code: 1, message: "Article ID is required." });
	}
...
export const updateArticle = async (req, res) => {
	...
	if (!id || !title || !content) {
		return res.status(400).json({
			code: 1,
			message: "Article ID, title, and content are required.",
		});
	}
	...
	try {
		...
		if (result.affectedRows === 0) {
			return res
				.status(404)
				.json({ code: 1, message: "Article not found or no changes made." });
		}
