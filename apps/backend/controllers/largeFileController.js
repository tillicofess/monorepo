...
const saveFileMetadata = async (
	...
	try {
		...
		if (existingFile.length > 0) {
			...
			console.log(
				`文件 ${fileName} (ID: ${existingFile[0].id}) 已存在于数据库，跳过插入。`,
			);
			return res.json({
				status: true,
				message: "文件已存在，无需重复上传及记录。",
			});
		}
...
export const createFolder = async (req, res) => {
	...
	if (!name || name.trim() === "") {
		return res
			.status(400)
			.json({ code: 1, message: "Folder name cannot be empty." });
	}
		...
		try {
			...
			if (parentRows.length === 0) {
				return res.status(404).json({
					code: 1,
					message: "Parent folder not found or not a directory.",
				});
			}
		} catch (error) {
			...
			return res
				.status(500)
				.json({ code: 1, message: "Failed to check parent folder." });
		} finally {
...
export const renameFolderOrFile = async (req, res) => {
	...
	if (!id || !name || name.trim() === "") {
		return res
			.status(400)
			.json({ code: 1, message: "ID and new name are required." });
	}
	...
	try {
		...
		if (result.affectedRows === 0) {
			return res
				.status(404)
				.json({ code: 1, message: "File or folder not found." });
		}
		...
		res
			.status(500)
			.json({ code: 1, message: "Failed to rename folder or file." });
	} finally {
		if (connection) connection.release();
	}
};
...
export const deleteFileOrFolder = async (req, res) => {
	...
	try {
		...
		if (idsToDelete.length > 0) {
			...
			console.log(
				`Deleted database metadata for IDs: ${idsToDelete.join(", ")}`,
			);
		}
		await connection.commit();
		res.json({
			code: 0,
			message: "File(s) and folder(s) deleted successfully.",
		});
	} catch (error) {
		...
		res.status(500).json({
			code: 1,
			message:
				"Failed to delete file(s) or folder(s). Transaction rolled back.",
		});
	} finally {
		if (connection) connection.release();
	}
...
export const moveFileOrFolder = async (req, res) => {
	...
	if (!draggedId || !newParentId) {
		return res
			.status(400)
			.json({ code: 1, message: "draggedId and newParentId are required." });
	}
	...
	try {
		...
		if (draggedRows.length === 0) {
			...
			return res
				.status(404)
				.json({ code: 1, message: "Dragged item not found." });
		}
		...
		if (normalizedParentId !== null) {
			...
			if (targetRows.length === 0) {
				...
				return res
					.status(400)
					.json({ code: 1, message: "Target folder not found." });
			}
			...
			if (!target.is_directory) {
				...
				return res
					.status(400)
					.json({ code: 1, message: "Target is not a directory." });
			}
			...
			if (String(draggedId) === String(normalizedParentId)) {
				...
				return res
					.status(400)
					.json({ code: 1, message: "Cannot move item into itself." });
			}
		...
		await connection.query(
			`UPDATE files_metadata SET parent_id = ? WHERE id = ?`,
			[normalizedParentId, draggedId],
		);
...
export const downloadFile = async (req, res) => {
	...
	try {
		...
		if (fileRows.length === 0) {
			return res
				.status(404)
				.json({ status: false, message: "文件不存在或无权限访问" });
		}
