...
export const createLog = async (req, res) => {
	try {
		...
		if (!type) {
			return res
				.status(400)
				.json({ success: false, message: "Missing type field" });
		}
		...
		if (type === "error") {
			...
			subType === "waterfall"
				? await insertWaterfallLog(body)
				: await handlePerformanceLog(body);
		} else {
			return res
				.status(400)
				.json({ success: false, message: "Unknown log type" });
		}
...
export const getUrlsByApp = async (req, res) => {
	try {
		...
		if (!appName) {
			return res
				.status(400)
				.json({ success: false, message: "Missing appName" });
		}
	...
	try {
		...
		if (!fse.existsSync(manifestPath)) {
			return res.status(404).json({
				success: false,
				message: "Manifest not found for this version",
			});
		}
		...
		if (!realMapFileName) {
			return res
				.status(404)
				.json({ success: false, message: "Map mapping not found in manifest" });
		}
		...
		if (!fse.existsSync(mapFilePath)) {
			return res
				.status(404)
				.json({ success: false, message: "Source map file not found" });
		}
